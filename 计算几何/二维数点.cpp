...
class Count_point_2D {
   public:
    void join_point(int x, int y, int val = 1) {
        a[++cnt] = {x + 1, y + 1, val, query_priority};
    }
    void join_suq(int x_1, int y_1, int x_2, int y_2) {
        ++x_2, ++y_2;
   ...
   private:
    ...
    inline void add(int x, int v, int ed) {
...
inline void solve() {
    ...
    for (int i = 1; i <= n; ++i) {
        int x, y, w;
        cin >> x >> y >> w;
        cp2d.join_point(x, y, w);
    }
    for (int i = 1; i <= m; ++i) {
        int x_1, y_1, x_2, y_2;
        cin >> x_1 >> y_1 >> x_2 >> y_2;
        cp2d.join_suq(x_1, y_1, x_2, y_2);
    }
    cp2d.out();
} 